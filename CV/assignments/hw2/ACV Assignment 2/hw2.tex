...
We applied the naive object counting pipeline to a video sequence containing 3--5 objects.
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Border artifacts:} After rotation, image corners contain regions outside the original frame. Using border replication fills these with edge pixels, which can create visual artifacts. Cropping to the largest inscribed rectangle would solve this but reduces the field of view.
\end{enumerate}
...
\begin{figure}[!ht]
    ...
    \caption{Compression results for the high-resolution satellite image. Left: Original. Center: Log-Magnitude Spectrum. Right: Compressed image retaining top 10\% of coefficients.}
...
\begin{figure}[!ht]
    ...
    \caption{Compression results for the low-frequency image. The compressed version retains high fidelity even with significant data removal.}
...
\begin{figure}[!ht]
    ...
    \caption{Compression results for the newly acquired satellite image. The algorithm preserves large-scale features while achieving compression.}
...
\begin{figure}[!ht]
    ...
    \begin{subfigure}[b]{0.19\textwidth}
        ...
        \caption{Shear Y}
    \end{subfigure}
    \caption{Data augmentation results applied to the image.}
...
\begin{itemize}
    \item \textbf{Photometric Distortion (Brightness/Contrast):} Adjusting brightness and contrast is crucial because face recognition systems must be robust to varying lighting conditions\cite{sun2005geometric}, such as shadows, indoor/outdoor transitions, and overexposure.
